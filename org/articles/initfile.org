...
The blog index is auto-generated before each publish run via a preparation function that calls =scripts/generate_blog_index.py=. This scans =org/blog/= for post files and rewrites the post list in =org/blog/index.org= so I don't have to maintain it manually.
