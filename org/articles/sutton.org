...
\bar{o}_n \doteq \bar{o}_{n-1} + \alpha (1 - \bar{p_{n-1}}) for n > 0, with \bar{o_0} \doteq 0.
...
$\pi_*(a | s) = \mathbbm{1}[q_*(s, a) = max_{a \in A}(q_*(s, a))]$

Exercise 3.28 Give an equation for \pi_* in terms of v_* and the four-argument p. ⇤

$\pi_*(a | s) = \mathbbm{1}[\sum_{s', r} p(s', r | s, a) [r + \gamma v_*(s')] = max_{a}(\sum_{s', r} p(s', r | s, a) [r + \gamma v_*(s')])]$
